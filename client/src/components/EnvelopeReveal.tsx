...
/**
 * ENVELOPE REVEAL — a calm, physical opening.
 * Tap the wax seal: the top flap opens, the card eases up, then the page is
 * revealed. The tap also starts the music (a real user gesture).
 */
...
export function EnvelopeReveal({ onReveal, onStartMusic }: EnvelopeRevealProps) {
  ...
  const handleOpen = () => {
    ...
    setOpen(true);
    onStartMusic();
    setTimeout(onReveal, reduce ? 60 : 1700);
  };
  ...
  return (
    ...
    >
      <div className="relative" style={{ perspective: 1200, width: "min(90vw, 420px)" }}>
        {/* Card that eases up out of the envelope */}
        <motion.div
          className="absolute inset-x-4 bottom-6 z-10 rounded-md px-6 py-8 text-center"
          ...
          transition={open ? SPRING : { duration: 0.3, ease: EASE }}
        >
          ...
          <p className="font-light" style={{ fontFamily: "var(--font-display)", fontSize: "2rem", color: "var(--paper)" }}>
            {couple.groomFirst} <span style={{ color: "var(--gold)" }}>&amp;</span> {couple.brideFirst}
          </p>
        ...
        <motion.button
          ...
          animate={open ? { scale: 0, opacity: 0 } : { scale: [1, 1.04, 1] }}
          transition={open ? { duration: 0.4, ease: EASE } : { duration: 2.4, repeat: Infinity, ease: "easeInOut" }}
          whileHover={{ scale: 1.06 }}
          whileTap={{ scale: 0.95 }}
        >
          <span style={{ fontFamily: "var(--font-display)", fontStyle: "italic", fontSize: "1.05rem", color: "#2a1f08" }}>
