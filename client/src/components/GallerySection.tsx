...
/**
 * GALLERY (blueprint §4) — polaroid cards (3:4, max 320px) on a draggable rail.
 * Framer drag="x" with stiff elasticity (0.05 desktop), momentum on touch.
 * Cards alternate −2°/+2°, carry the spec shadow + a thin wood border, and open
 * a crossfading, keyboard-driven lightbox. Placeholders until photos are added.
 */
export function GallerySection() {
  ...
  // Compute how far the rail can travel
  useEffect(() => {
    const measure = () => {
      const el = wrapRef.current;
      if (!el) return;
      setConstraint(Math.max(0, el.scrollWidth - el.clientWidth));
    };
    measure();
  ...
  return (
    <section id="gallery" className="relative w-full overflow-hidden py-24 md:py-32 lg:py-40">
      <div className="mb-12 px-6 text-center md:mb-16 md:px-8">
        ...
        <p className="mt-4 text-[0.8rem]" style={{ color: "var(--paper-dim)" }}>Drag to explore →</p>
      </div>
        ...
        >
          {photos.map((photo, index) => (
            <motion.button
              ...
              style={{ width: "min(74vw, 280px)", transform: `rotate(${index % 2 === 0 ? -2 : 2}deg)` }}
            ...
            >
              ...
              <div
                ...
                style={{
                  ...
                  border: "1px solid rgba(120,85,40,0.45)",
                  boxShadow: "0 20px 40px rgba(0,0,0,0.5), 0 1px 3px rgba(255,255,255,0.1) inset",
                }}
              >
                <div className="overflow-hidden" style={{ aspectRatio: "3 / 4" }}>
                  <Photo src={photo.image} alt={photo.title} className="h-full w-full" imgClassName="pointer-events-none" label={`gallery-${index + 1}.jpg`} />
                </div>
      ...
      <AnimatePresence>
        {selected !== null && (
          ...
          >
            ...
            >
              <Photo src={photos[selected].image} alt={photos[selected].title} className="max-h-[78vh] w-[min(86vw,640px)]" imgClassName="!h-auto max-h-[78vh]" label={`gallery-${selected + 1}.jpg`} />
            </motion.div>
